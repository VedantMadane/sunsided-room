        ...
        if !((*check).flags as c_int & ML_TWOSIDED == 0) {
            ...
            if !(openrange <= 0 as c_int) {
                ...
                eprintln!("RSOUND_STOP lt={} sec={} secs=[{},{}] openrange={} floor0={} ceil0={} floor1={} ceil1={}",
                    ...
                    (*other_sec1).floorheight, (*other_sec1).ceilingheight);
            }
        }
        i += 1;
    }
...
pub unsafe extern "C" fn P_LookForPlayers(
    ...
    let _lt_lfp = crate::doom::p_tick::leveltime;
    let _dbg_lfp = _lt_lfp >= 2775 && _lt_lfp <= 2785;
    if _dbg_lfp {
        eprintln!(
                    ...
                    if !(sight == 0) {
                        if allaround == 0 {
                            ...
                            if an > ANG90 as angle_t && an < ANG270 {
                                ...
                                if _dbg_lfp {
                                    eprintln!(
                                        "LFP BEHIND lt={} addr={:p} dist={} meleerange={}",
                                        _lt_lfp, actor, dist, MELEERANGE
                                    );
                                }
                                if dist > MELEERANGE {
                                    break 's_20;
...
pub unsafe extern "C" fn A_Look(mut actor: *mut mobj_t) {
    ...
    let _lt_alook = crate::doom::p_tick::leveltime;
    if _lt_alook >= 2775 && _lt_alook <= 2785 {
        ...
        if LAST_LT_LOGGED.load(std::sync::atomic::Ordering::Relaxed) != _lt_alook {
            ...
            if playeringame[0] != 0 {
                let pmobj = (*players.as_ptr()).mo as *const mobj_t;
                if !pmobj.is_null() {
                    eprintln!(
                        "PLAYER lt={} x={} y={} z={} angle=0x{:x}",
                        _lt_alook,
                        (*pmobj).x,
                        (*pmobj).y,
                        (*pmobj).z,
        ...
        eprintln!(
            "ALOOK lt={} type={} addr={:p} ax={} ay={} flags=0x{:x} soundtgt={:p} tgt_shootable={} ambush={}",
            _lt_alook, (*actor).mobjtype, actor,
            (*actor).x, (*actor).y,
            (*actor).flags,
            targ,
            targ_shootable,
...
pub unsafe extern "C" fn A_Chase(mut actor: *mut mobj_t) {
    ...
    if _lt_achase >= 2775 && _lt_achase <= 2785 {
        ...
        eprintln!(
            ...
            (*actor).flags, (*actor).movecount,
            (*actor).target, tgt_health,
            if (*actor).target.is_null() { 0 } else { (*(*actor).target).flags as u32 },
        );
    }
